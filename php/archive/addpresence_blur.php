...
//{"ip":"24.114.106.140","country_code":"CA","country_name":"Canada","region_code":"QC","region_name":"Quebec","city":"","zip_code":"","time_zone":"America/Toronto","latitude":45.5,"longitude":-73.5833,"metro_code":0}
	...
	BEGIN
		...
		BEGIN	
			   ...
			   VALUES ('".$_SERVER["SERVER_NAME"]."',?,'false','".$currentdatestring."','0', ".$geoip_json->longitude.",".$geoip_json->latitude." ,  '".$geoip_json->{'country_name'}."',  '".$geoip_json->city."',  '".$geoip_json->ip."')
		END
		ELSE
		BEGIN
			UPDATE dbo.dotr_presence
			SET domain = '".$_SERVER["SERVER_NAME"]."',focus = 'false',lastpresence = '".$currentdatestring."'
			WHERE uniqueid = ?
		END  
	END		   
	";
